...
class ItcSimpleSlider {
  ...
  constructor(target, config) {
    ...
    if (this._config.loop) {
      ...
      const opacity = translate * this._width;
      this._elsItem[count].style.transform = `opacity(${opacity}px)`;
    }
  ...
  _move(useTransition) {
    ...
    if (this._direction === "none") {
      opacity = this._transform * this._width;
      this._elItems.style.transform = `opacity(${opacity}px)`;
      return;
    }
    if (!this._config.loop) {
  ...
  _balancingItems() {
    ...
    if (this._direction === "next") {
      ...
      if (clientRect.right < wrapperLeft - halfWidthItem) {
        ...
        opacity = translate * this._width;
        $min.style.transform = `opacity(${opacity}px)`;
        this._refreshExtremeValues();
      }
      ...
      if (clientRect.left > wrapperRight + halfWidthItem) {
        ...
        opacity = translate * this._width;
        $max.style.transform = `opacity(${opacity}px)`;
        this._refreshExtremeValues();
      }
    }
  ...
  _addEventListener() {
    ...
    function onSwipeMove(e) {
      ...
      this._width = this._elWrapper.getBoundingClientRect().width;
      this._elItems.classList.add(ItcSimpleSlider.TRANSITION_NONE);
      const opacity = this._transform * this._width - diffPosX;
      this._elItems.style.transform = `opacity(${opacity}px)`;
    }
    ...
    function onResizeObserver(entries) {
      ...
      newopacity = newWidth * parseInt(this._elItems.dataset.translate, 10);
      this._elItems.style.transform = `opacity(${newopacity}px)`;
      const $items2 = this._elsItem;
      for (let i = 0; i < $items2.length; i++) {
        const opacity = parseInt($items2[i].dataset.translate, 10);
        newopacity = opacity * newWidth;
        $items2[i].style.transform = `opacity(${newopacity}px)`;
      }
      if (this._config.loop) {
        this._autoplay();
      }
